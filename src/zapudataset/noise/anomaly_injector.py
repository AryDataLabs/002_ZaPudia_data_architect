...
"""
Real-world noise & anomaly injection, vectorized with Polars.
Five injectors, applied in the fixed order:
...
def inject_schema_degradation(
    ...
    """
    Null category_code and brand for 
    cfg.schema_null_rate of items.
    Applied at item level; the same item's 
    metadata is nulled across all its
    events. Returns the degraded frame plus 
    the set of nulled item_ids.
    """
    rng        = _rng(cfg.seed)
...
def partition_cold_items(
    ...
    """
    Select cfg.cold_start_rate of active 
    items as cold (zero-history).
    Returns (cold_item_ids, warm_events, 
    cold_item_catalog_rows).
    Warm events = events whose item is NOT cold;
    cold-item events are removed
    from the training stream but retained 
    in the item catalog.
    """
    rng          = _rng(cfg.seed + 1)
    active_items = events["item_id"].unique().to_numpy()
    n_cold       = int(round(cfg.cold_start_rate * len(active_items)))
    cold_idx     = rng.choice(len(active_items), 
                    size = n_cold, replace = False)
    cold_items   = set(active_items[cold_idx].tolist())
    warm_events  = events.filter(~pl.col("item_id").is_in(list(cold_items)))
    cold_catalog = events.filter(pl.col("item_id").is_in(
                   list(cold_items))).unique(
                   subset         = ["item_id"], 
...
def inject_out_of_order(
    ...
    """
    Add exponential jitter to event_time; 
    retain true_event_time for eval.
    """
    rng       = _rng(cfg.seed + 2)
    n         = events.height
    jitter_ms = rng.exponential(
                scale = 1000.0 / cfg.oo_lambda_per_sec,
                size  = n).astype(np.int64)
    jitter_ms = np.clip(jitter_ms, 0, None)
    return events.with_columns(
        pl.col("event_time").alias("true_event_time")
        ).with_columns(
        (pl.col("event_time") + pl.Series(jitter_ms)
        ).alias("event_time"))

def inject_telemetry_drops(
        events : pl.DataFrame, 
        cfg    : NoiseConfig,
    ) -> pl.DataFrame:
    """
    Drop cfg.telemetry_drop_rate of cart events 
    that precede a purchase in
    the same (user, item) session.
    The drop is conditioned on a subsequent 
    purchase existing, isolating telemetry 
    loss from genuine abandonment.
    """
    rng      = _rng(cfg.seed + 3)
    enriched = events.with_columns(
               pl.col("event_type").alias("_et"),
               pl.col("event_time").alias("_t"),)
    carts    = (enriched.sort(["user_id", "item_id", 
                               "user_session", "event_time"])
               .with_columns(pl.col("event_time")
    ...
    drop_keys = (carts.with_columns(
                 pl.Series(drop_mask).alias("_drop"))
                 .filter(pl.col("_drop"))
...
def inject_bot_flood(
    ...
    """
    Inject synthetic bot streams with 
    sub-10ms inter-arrival times.
    Bot target items sampled from a 
    power-law popularity distribution.
    Bots are anonymous (user_id NULL, 
    user_pseudo_id set) so they must be
    ...
    pop       = (events.group_by("item_id")
                .agg(pl.len().alias("p"))
                .with_columns((pl.col("p") ** 0.75).alias("weight")))
    weights   = pop["weight"].to_numpy()
    weights   = weights / weights.sum()
    items     = pop["item_id"].to_numpy()
    bot_items = rng.choice(items, 
                           size = n_bot_events, 
                           p    = weights,)

    # number of bots and events-per-bot
    n_bots    = max(1, n_bot_events // 200)
    eventsbot = n_bot_events // n_bots
    pseudo_id = [f"bot_{cfg.seed}_{i}" for i in range(n_bots)]
    rows      : list[dict] = list()
    base_t    = int(events["event_time"].min())
    for b in range(n_bots):
        pid   = pseudo_id[b]
        ne    = eventsbot if b < n_bots - 1 else n_bot_events - b * eventsbot
        iats  = rng.exponential(
                scale = 1000.0 / cfg.bot_lambda_per_sec, 
                size  = ne,)
        iats  = np.clip(iats, 
                cfg.bot_interarrival_min_ms, 
                cfg.bot_interarrival_max_ms,
                ).astype(np.int64)
        ts    = base_t + np.cumsum(iats)
        item_slice = bot_items[b * eventsbot : b * eventsbot + ne]
